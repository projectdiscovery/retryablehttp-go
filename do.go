...
func (c *Client) Do(req *Request) (*http.Response, error) {
	...
	for i := 0; ; i++ {
		...
		if c.HTTPClient2 != nil && err != nil && stringsutil.ContainsAny(err.Error(), "net/http: HTTP/1.x transport connection broken: malformed HTTP version \"HTTP/2\"", "net/http: HTTP/1.x transport connection broken: malformed HTTP response") {
