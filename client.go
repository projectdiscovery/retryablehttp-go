...
type Options struct {
	...
	// DisableHTTP2 disables the HTTP/2 fallback mechanism. When enabled,
	// the client will not fall back to HTTP/2 upon receiving a malformed
	// HTTP version error from an HTTP/1.x transport connection.
	DisableHTTP2 bool
}
